...
def img_import(img_path: str) -> tuple:
...
def get_photo_contour(contours: tuple, hierarchy: np.ndarray) -> np.ndarray:
    ...
    if len(photos) > 1:
        print(
            "photos length is greater than 1.... possibly more than one photo? multiple photos code not implimented yet"
        )
        return
    else:
        return photos
